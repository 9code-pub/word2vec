...
public class SortInputFormat extends FileInputFormat<Text, Text> {
				...
				public void run() {
					...
					try {
						...
						RecordReader<Text, Text> reader = inFormat
								.createRecordReader(splits.get(sampleStep * idx), context);
						...
						System.err.println("Got an exception while reading splits " + StringUtils
	...
	public List<InputSplit> getSplits(JobContext job) throws IOException {
		...
		if (job.getConfiguration().getBoolean(SortSpliter.USE, true)) {
			SortSpliter spliter = new SortSpliter(lastResult.toArray(new FileSplit[0]), job.getConfiguration());
			lastResult = spliter.reSplit();
			t3 = System.currentTimeMillis();
			System.out.println("Spent " + (t3 - t2) + "ms computing SortSpliter splits.");
		}
		return lastResult;
	}
}
